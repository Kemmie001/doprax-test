...
<template>
  ...
  >
    <div
      class="bg-white text-center p-4 py-5 md:px-6 h-[80vh] md:h-[100vh] shadow-lg max-w-[90%] md:max-w-md w-full"
    >
      <div class="overflow-y-auto h-full">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex items-center gap-6">
            ...
            >
              <img src="../assets/close-icon.svg" alt="close" class="w-5 h-5" />
            </button>
          </div>
        </div>
          ...
          >
            ...
            <div class="text-left">
              <p class="text-xs font-normal text-primary">
                ...
                {{ formStore?.deploymentDetails?.resources.memory }} GB Memory,
                {{ formStore?.deploymentDetails?.resources.storage }} GB Storage
              </p>
            </div>
          </div>
          <div
            class="px-5 py-4 md:py-7 md:border-b items-start border-gray-200 flex flex-col gap-2 md:gap-4"
          >
            <p class="text-gray-100 text-xs leading-md font-medium">Network</p>
            <div class="text-left">
  ...
  <!-- <Transition name="modal">
    <div
      v-if="show"
      class="fixed inset-0 z-50 bg-[#00000040] flex items-end md:items-center justify-center"
      @click.self="emitClose"
    >
      <div
        class="max-w-[90%] w-full md:w-[400px] md:h-[100vh] bg-white rounded-t-lg md:rounded-l-lg p-4"
        :class="[
      ...
      >
        <div class="flex justify-between items-center mb-4">
          ...
          <button @click="emitClose" class="text-gray-500 hover:text-black">
            ✕
          </button>
        </div>
        <slot />
